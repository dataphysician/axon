...
# ──────────────────────────────────────────────────────────────────────────────
# dataclass (generic type) used ONLY when reporting a node (not stored in index)
# ──────────────────────────────────────────────────────────────────────────────
@dataclass(slots=True)
class GenericNode[T: Hashable]:
    """
    ...
    def __new__(cls, s: str) -> ICDCode:
        ...
        if not m:
            raise ValueError(
                ...
                "'A00-B99', 'C01', 'E11.621' or 'S06.0X1'."
            )
        g = m.groupdict()
        if g["root"]:
...
if __name__ == "__main__":
    ...
    for nid in [
        ...
        "ROOT", "Chapter_04", "B35-B49", "S06.0X1", 123
    ]:
        try:
            view_node(str(nid))
